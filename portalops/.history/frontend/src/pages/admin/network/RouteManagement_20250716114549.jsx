...
export default function RouteManagement() {
    ...
    const handleCreateRoute = () => {
        if (!formData.name || !formData.router_id || !formData.destination_cidr || !formData.next_hop) {
        ...
        const newRoute = {
            id: `route-${Date.now()}`,
            ...formData,
            router_name: routers.find(r => r.id === formData.router_id)?.name || '',
            status: 'active'
        };
        ...
        const updatedRoutes = routes.map(route =>
            route.id === selectedRoute.id
                ? {
                    ...route,
                    ...formData,
                    router_name: routers.find(r => r.id === formData.router_id)?.name || route.router_name
                }
                : route
        );
    ...
    const routerOptions = routers.map(router => ({
        value: router.id,
        label: router.name
    }));
